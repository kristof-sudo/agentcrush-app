-- Migration: 20260608_1000_restore_model_family_v14
-- URGENT RESTORATION — fixes regression introduced by 20260608_0920.
--
-- The earlier same-day migration (20260608_0920_model_family_scoring_view.sql)
-- did `DROP VIEW IF EXISTS public.agent_score_model_family_v1 CASCADE` and
-- replaced it with a simpler v1.0 view that lacked columns the /rankings/model-families
-- page depends on (hf_author, total_downloads, hf_score, derivatives_score,
-- lmarena_score, citations_score, deployment_score). The CASCADE also dropped
-- the dependent wrapper view `agent_score_model_family_v1_with_confidence`.
--
-- This migration restores both views exactly as defined in:
--   20260516_1200_plumb_deployment_into_model_family_view.sql  (v1.4-FULL)
--   20260523_0753_score_confidence_tier.sql                     (wrapper)
--
-- The big-10 seed (20260608_0900) added new rows (openai, anthropic_claude,
-- xai_grok) that will appear in the restored v1.4 view but score 0 because they
-- have no hf_author / lmarena_model_keys / semantic_scholar_paper_ids /
-- model_family_deployments rows. They're correctly indexed but unranked under
-- v1.4 until we ingest cross-protocol signals for closed-weights families.
-- (TODO: backfill lmarena_model_keys for gpt-4o, claude-3-5-sonnet, grok-2.)
--
-- Idempotent.
...
-- ── Restore confidence-tier wrapper view ──────────────────────────────────────
...
COMMENT ON VIEW public.agent_score_model_family_v1_with_confidence IS
  'Wrapper over agent_score_model_family_v1 (v1.4) adding a confidence_tier derived from signal coverage. Tier values: high (5/5), medium (4/5), low (3/5 — still evidence-ready), provisional (<3/5 — not evidence-ready).';
