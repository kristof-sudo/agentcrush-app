...
export default async function GhostIndexPage() {
  ...
  const totalAgents    = current?.total_agents    ?? 1438
  const aliveAgents    = current?.alive_agents    ?? 760
  const ghostAgents    = current?.ghost_agents    ?? (totalAgents - aliveAgents)
  const evidenceRanked = current?.evidence_ranked ?? 194
  const indexedOnly    = current?.indexed_only    ?? 1244
