...
async function likeXTweet(tweetId) {
  ...
  if (!res.ok) {
    // NEVER map 403 to success: X returns 403 both for duplicate likes AND for
    // permission-denied. Two days of likes silently failed (read-only tokens)
    // while logging "(already liked)". Duplicates return 200 with liked:true,
    // so any non-2xx is a real failure — log the body, fail loudly.
    const body = await res.text().catch(() => '')
    throw new Error(`HTTP ${res.status}: ${body.slice(0, 200)}`)
  }
  const data = await res.json().catch(() => null)
  return { alreadyLiked: data?.data?.liked === true && data?.data?.already_liked === true }
}
