-- agent_hn_signal_rollups: tighten candidate score scaling for HN evidence.
-- Analysis only. Does not write agent rows or affect rankings.

CREATE OR REPLACE VIEW agent_hn_signal_rollups AS
WITH relevant AS (
  SELECT
    agent_id,
    source_url,
...
SELECT
  ...
  LEAST(
    100,
    ROUND((
      (rollups.recent_90d_log_strength / LN(1 + 10000) * 70) +
      (rollups.log_strength / LN(1 + 10000) * 30)
    )::numeric, 2)
